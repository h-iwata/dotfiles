module Shared::PriceCalculation
  ...
  def calculate_price(parent_status, fixed_cancel_fee = nil)
    ...
    parent_status.student_statuses.each_with_index do |ss, index|
      ss_price = calculate_student_status_price(ss, apply_date, parent_status.preentried_camp, parent_status.parent)
      # キャンセル料の計算
      ...
      cancel ||= -> {
        ...
        if parent_status.invoices.to_a.all? { |invoice| invoice.verified_at.blank? } || !ss.cancelled? || !last_invoice
          return {
            cancel_fee: 0,
            cancel_rate: 0,
    ...
    # 兄弟のトータル額
    total_price = ss_prices.values.inject(0) { |sum, ss_price| sum + ss_price[ss_price[:status] == "cancelled" ? :cancel_fee : :total] }

    # 割引を差し引いた料金
    price_before_tax = parent_status.cancelled? ? total_price : total_price + total_discount
    price_before_tax = 0 if price_before_tax < 0

    # 消費税
    tax = CalculationHelper::calculate_tax(price_before_tax)
    result = {
      price: adjusted_price(price_before_tax + tax, parent_status),
      tax: tax,
  ...
  def adjusted_price(price_before_adjustment, parent_status)
    ...
    price = price_before_adjustment > adjustment_payment ? price_before_adjustment - adjustment_payment : 0
    Rails.logger.info "Manual adjustment: #{adjustment_payment}\n" + 'Appyling manual adjustment.' if adjustment_payment && adjustment_payment != 0
    return price
  end

  def calculate_student_status_price(ss, apply_date, preentry_camp, parent)
    price = 0
    early_discount = 0
    travel_cost = 0
    if ss.plan_stayplan
    ...
    total = 0 + price + pc_rental_fee + early_discount + travel_cost
    total += rental_prices[:total_price] if rental_prices.has_key? :total_price

    if preentry_camp.present?
      pp preentry_camp.discount(total, parent)
