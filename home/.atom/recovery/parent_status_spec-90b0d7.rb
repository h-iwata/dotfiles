...
RSpec.describe ParentStatus, type: :model do
  ...
  describe "#search" do
    ...
    context 'with blank words' do
      its[''] { expect(subject).to eq(1) }
    end

    context 'with parent_name' do
      its['shimpei'] { expect(subject).to eq(1) }
    end

    context 'with student_name' do
      its['sisopei'] { expect(subject).to eq(1) }
    end

    context 'with mismatch words' do
      its['komosama shimpei'] { expect(subject).to eq(0) }
    end
  end
  ...
  describe '#coupons' do
    ...
    context 'with inactive coupon' do
      ...
      its(:coupons) { is_expected.to have(1).items }
      its(:errors) { is_expected.to have(1).items }
