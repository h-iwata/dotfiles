...
RSpec.describe ParentStatus, type: :model do
  ...
  describe "#search" do
    subject { described_class.search(by_name_or_email_or_phone: query).result.count }
    ...
    context 'with parent_name' do
      let(:query) { 'shimpei' }

      it { is_expected.to eq(1) }
    end

    context 'with student_name' do
      let(:query) { 'sisopei' }

      it { is_expected.to eq(1) }
    end

    context 'with mismatch words' do
      let(:query) { 'komosama shimpei' }
  ...
  describe '#coupons' do
    ...
    context 'with camp coupon' do
      ...
      it { expect(ps.coupons.length).to eq 1 }
      it { expect(ps.coupons_are_valid?(true)).to be true }
      it { expect(ps.errors.count).to eq 0 }
      it { expect(ps.coupons_are_valid?(false)).to be true }
    end
    ...
    context 'with non camp coupon' do
      ...
      its(:errrors) { is_expected.to be_empty }
