...
RSpec.describe CampCancelRate, type: :model do
  describe '#cancel_rate' do
    ...
    context "with plan.cancel_fee_start_date" do
      subject do
      ...
      it do
        expect(plan.cancel_fee_start_date).not_to be nil
      end

      it { is_expected.to eq(BigDecimal("50.00")) }
    end

    it 'camp.cancel_fee_start_date' do
      plan = camp.plans[0]
      expect(plan.cancel_fee_start_date).to be nil
      expect(camp.cancel_fee_start_date).not_to be nil
      expect(described_class.cancel_rate(plan, camp.cancel_fee_start_date - 1)).to eq(BigDecimal("0"))
      expect(described_class.cancel_rate(plan, camp.cancel_fee_start_date)).not_to eq(BigDecimal("0"))
    end
